...
async function renderInner(wrapped, data) {
	...
	if (options.updateTokenImageEnable) {
		html.find("footer > button").before('<div class="form-group"><input type="checkbox" id="update-token-image" name="update-token-image"/><label for="update-token-image">Update Token Image</label></div>');
